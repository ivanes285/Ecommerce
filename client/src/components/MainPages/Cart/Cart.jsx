  ...
  useEffect(() => {
    const getTotal=() => {
      const total = cart.reduce((prev,item)=>{
         return prev+(item.price*item.quantity)  //sumamos el total recorriendo el array de productos 
      },0)
      setTotal(total)
    }
    getTotal()
...
const removeProduct = (id) => {
  ...
  if(window.confirm('Do you want to delete this product?')){
    ...
    setCart([...cart]) 
    addToCart(cart) //llamando para actualizar el carrito
  }
}
 ...
 await  axios.post('/api/payment',{cart,paymentID,address },{headers: {Authorization: token }})
  ...
  return (
    <div>
      ...
      <div className="total">
        <h3>Total: $ {total}</h3>
         <PaypalButton totalValor={total} tranSuccess={tranSuccess}/> 
      </div> 
